...
// function customer_price($atts, $content = null) {
//     $price = $atts['price'] ?? null;
//     $cookieAlias = $atts['if_cookie'] ?? 'regular';

//     $clientCookie = $_COOKIE['shared'];

//     if ($cookieAlias === $clientCookie) {
...
function customer_price() {
    $clientPartnerId = $_COOKIE['partnerRef'] ?? null;

    return get_price($clientPartnerId);
}

add_shortcode('customer_price', 'customer_price');
